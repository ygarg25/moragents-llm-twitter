...
import sys
# set sys path to hub
sys.path.append(os.path.dirname(os.path.abspath(__file__)) + '/hub')
...
from core.config import SUMMARIES_FILE, TWEETS_ARCHIVE, ANTHROPIC_API_KEY, FARCASTER_SIGNER_KEY, FID
...
from hub.client import cast_message, submit_message
...
async def generate_and_post_tweet():
    ...
    try:
        ...
        response = submit_message(cast_message(
            tweet_text, 
            FARCASTER_SIGNER_KEY, 
            FID
        ))
        logger.info(f"Post Transaction Hash: {response}")
